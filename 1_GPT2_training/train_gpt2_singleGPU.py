...
for step in range(max_steps):
    ...
    if step % 250 == 0 or last_step:
        ...
        print(f"Validation loss: {val_loss_accum/20:.4f}")
    ...
    if (step % 250 == 0 or last_step) and (not use_compile):
        ...
        print(f"HellaSwag acc: {num_correct_norm/num_total:.4f}")

    # Generation
    if ((step > 0 and step % 250 == 0) or last_step) and (not use_compile):
        model.eval()
        tokens = torch.tensor(enc.encode("Hello, I'm a language model,"), dtype=torch.long).unsqueeze(0).repeat(4, 1).to(device)
        rng = torch.Generator(device=device).manual_seed(42)
        while tokens.size(1) < 32:
            with torch.no_grad(), torch.autocast(device_type=device_type, dtype=torch.bfloat16):
                logits, _ = model(tokens)
        ...
        for i in range(4):
            print(enc.decode(tokens[i, :32].tolist()))
    ...
    dt = time.time() - t0
    print(f"Step {step} | loss: {loss_accum/grad_accum_steps:.4f} | lr: {lr:.4e} | {dt*1000:.2f} ms")
